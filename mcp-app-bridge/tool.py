...
author: Classic298
version: 0.6.0
description: Wraps MCP server tools and renders MCP App UI resources (ui://) as Rich UI embeds using Open WebUI's existing embed system. Spec-compliant: honors server-declared CSP. By default dispatches a synthetic ui/notifications/tool-result for AppBridge SDK compatibility (no middleware changes needed). Optionally (Valve 'host_bridge') emits a marker for a parent-window loader.js that answers the ui/initialize handshake, enabling full interactivity (tool calls, links, messages).
"""
...
def _build_bridge_marker(config: dict) -> str:
    """Build the HTML comment marker read by the host-side ``static/loader.js``.

    The loader (running in Open WebUI's parent window) scans iframe ``srcdoc``
    for ``<!--__MCP_APP_BRIDGE__ <base64-json> -->`` and uses the decoded
    config to answer the App's ``ui/initialize`` handshake and deliver the tool
    input/result via the real MCP-UI protocol.
    """
    payload = json.dumps(config, ensure_ascii=False).encode("utf-8")
    encoded = base64.b64encode(payload).decode("ascii")
    return f"<!--__MCP_APP_BRIDGE__ {encoded} -->\n"
...
class Tools:
    ...
    class Valves(BaseModel):
        ...
        host_bridge: bool = Field(
            default=False,
            description=(
                "Enable the host-side AppBridge. Requires 'static/loader.js' "
                "(examples/open-webui-loader) installed in Open WebUI. When on, "
                "the App's ui/initialize handshake is answered in the parent "
                "window so full interactivity works (tool calls, links, "
                "messages), and the legacy synthetic tool-result shim is "
                "disabled to avoid duplicate delivery. Leave off if loader.js "
                "is not installed."
            ),
        )
        browser_mcp_url: str = Field(
            default="",
            description=(
                "Browser-reachable Streamable-HTTP URL of the MCP server, used "
                "by loader.js to proxy live tool calls (e.g. button clicks). "
                "Only used when 'host_bridge' is on. Leave empty to disable "
                "live server-tool calls. Note: the browser connects directly "
                "and does NOT send 'auth_token' — the server must allow CORS "
                "and either be unauthenticated or sit behind a browser-auth "
                "proxy."
            ),
        )
    ...
    async def call_mcp_tool(
        ...
        try:
            ...
            if self.valves.host_bridge:
                # Host-bridge mode: the parent-window loader.js answers the
                # ui/initialize handshake and delivers the tool input/result via
                # the real protocol. Emit a marker it can read, and drop the
                # synthetic shim to avoid duplicate/out-of-order tool-result.
                try:
                ...
                bridge_config = {
                    ...
                    "maxHeight": int(max_height) if max_height else None,
                    "serverUrl": self.valves.browser_mcp_url or None,
                }
                marker = _build_bridge_marker(bridge_config)
                injection = (
                    csp_tag + marker + min_height_style + data_script + height_script
                )
            else:
                injection = (
                    csp_tag
