...
function GetTimeAppInner({ app, toolResult, hostContext }: GetTimeAppInnerProps) {
  ...
  const [messageText, setMessageText] = useState(
    "Read the attached resource and reply with the verification code it contains.",
  );
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    // A distinctive token the model can only know if the resource reached its
    // context. Logged here so you can compare it against the model's reply.
    const verificationCode = `MCP-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    console.info("Resource verification code:", verificationCode);
    try {
      const { isError } = await app.sendMessage(
        {
          ...
          content: [
            { type: "text", text: messageText },
            {
              type: "resource",
              resource: {
                uri: "mcp-app://get-time/notes.md",
                mimeType: "text/markdown",
                text: `# Notes from the Get Time App\n\nVerification code: ${verificationCode}\n\nShared at ${new Date().toISOString()}. Last known server time: ${serverTime}.`,
              },
            },
          ],
        },
